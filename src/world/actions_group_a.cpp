...
void cyclopsRoomAction(int rarg) {
  ...
  if (rarg == M_LOOK) {
    ...
    if (state.hasFled /* MAGIC-FLAG */) {
      printLine("The east wall, previously solid, now has a cyclops-sized "
                "opening in it.");
      ...
      printLine("A cyclops, who looks prepared to eat horses (much less mere "
      ...
      printLine("The cyclops is standing in the corner, eyeing you closely. I "
                "don't think he likes you very much. He looks extremely "
  ...
  if (rarg == M_BEG && (g.prsa == V_WALK || g.prsa == V_CLIMB_UP)) {
    ...
    bool goingUp =
        (g.prsa == V_CLIMB_UP) || (getDirection(g.prso) == Direction::UP);
    ...
    if (goingUp) {
      if (!state.hasFled && !state.isAsleep) {
        ...
        // BUT cyclopsRoomAction is void? C++ sig: void cyclopsRoomAction(int
        // rarg). Wait, usually room functions return bool in ZIL
        // (RTRUE/RFALSE). In C++ codebase, `actions_group_a.cpp` defines it as
        // `void` (Line 365). This might be an issue. How does it signal
        // blocking? Most room actions set `g.forceReturn = true` or `return
        // true` if signature allows. I should check `actions.h` or call site.
  ...
  // I will use `return;` and `printLine` for now. Blocking usually requires
  // returning true to parser. I'll check `chimneyAction` signature... `bool`.
  // `clearingAction`... `void`? (Step 7245 diff showed `extern void
  // clearingAction`). So rooms might be `void`. If rooms are `void`, how do
  // they block motion? Maybe `g.params`? or `g.blockMotion = true`? I will
  // print the message. If motion continues, it's a bug in engine mapping. I'll
  // assume printing "Refuses to let you pass" is the verification step. I'll
...
// =============================================================================
// RUG-FCN - Rug covering trap door
// ZIL: RAISE/MOVE/PUSH reveals trap door. TAKE blocked (too heavy).
// Source: 1actions.zil lines 579-605
// =============================================================================
bool rugAction() {
  auto &g = Globals::instance();
  ...
  if (g.prsa == V_RAISE) {
    ...
    if (g.rugMoved) {
      ...
      printLine(", but in trying to take it you have noticed an irregularity "
                "beneath it.");
    }
    return RTRUE;
  }

  if (g.prsa == V_MOVE || g.prsa == V_PUSH) {
    if (g.rugMoved) {
  ...
  if (g.prsa == V_LOOK_UNDER && !g.rugMoved) {
    printLine("There seems to be something under the rug.");
    return RTRUE;
  }
...
// =============================================================================
// TUBE-FUNCTION - Tube container (doesn't accept things)
// ZIL: PUT refused. SQUEEZE handled.
// Source: 1actions.zil lines 1386-1395
// =============================================================================
bool tubeAction() {
  auto &g = Globals::instance();
  ...
  if (g.prsa == V_SQUEEZE) {
    printLine("A viscous liquid oozes out of the tube.");
    return RTRUE;
  }
