    ...
    if (g.prso) {
      ...
      if (tValue > 0) {
        ...
        if (!scoreSystem.isTreasureScored(g.prso->getId())) {
          ...
          // But standard Zork transcript often just says "Done." and adds score
          // silently? Let's rely on standard vPut "Done." message by returning
          // RFALSE.
        }
      }
    }
...
bool bottleAction() {
  ...
  if (spilled) {
    ...
    if (water)
      water->moveTo(nullptr); // Remove water
    return true;
  }
...
bool rugAction() {
  ...
  if (g.prsa == V_MOVE || g.prsa == V_PUSH || g.prsa == V_PULL) {
    if (g.rugMoved) {
      printLine("Having moved the carpet previously, you find it impossible to "
                "move it again.");
    } else {
      printLine("With a great effort, the rug is moved to one side of the "
                "room, revealing the dusty cover of a closed trap door.");
  ...
  if (g.prsa == V_EXAMINE || g.prsa == V_LOOK_INSIDE) {
    if (!g.rugMoved) {
      printLine(
          "Underneath the rug is a closed trap door. As you drop the corner of "
          "the rug, the trap door is once again concealed from view.");
    } else {
...
bool wrenchAction() {
  ...
  if (g.prsa == V_EXAMINE && g.prso && g.prso->getId() == ObjectIds::WRENCH) {
    printLine("The wrench is a heavy metal tool, suitable for turning bolts "
              "and nuts.");
    return RTRUE;
  }
...
bool screwdriverAction() {
  ...
  if (g.prsa == V_EXAMINE && g.prso &&
      g.prso->getId() == ObjectIds::SCREWDRIVER) {
    printLine("The screwdriver is a metal tool with a flat blade, suitable for "
              "turning screws.");
    return RTRUE;
  }
...
// ZIL: Only handles MUNG to destroy the painting's value and change description
// Source: 1actions.zil lines 2207-2214
bool paintingAction() {
  auto &g = Globals::instance();
  ...
  // Handle MUNG - destroys painting value (ZIL spec)
  if (g.prsa == V_MUNG) {
    // Set TVALUE to 0 (worthless)
    g.prso->setProperty(P_TVALUE, 0);
    // Change LDESC to "worthless piece of canvas"
    g.prso->setLongDesc("There is a worthless piece of canvas here.");
    printLine("Congratulations! Unlike the other vandals, who merely stole the "
...
bool eggAction() {
  ...
  if (g.prsa == V_OPEN) {
    ...
    if (g.prsi->hasFlag(ObjectFlag::WEAPONBIT) ||
        g.prsi->hasFlag(ObjectFlag::TOOLBIT)) {
      printLine("The egg is now open, but the clumsiness of your attempt has "
    ...
    printLine("The concept of using a " + g.prsi->getDesc() +
  ...
  if (g.prsa == V_CLIMB_ON) {
    printLine("There is a noticeable crunch from beneath you, and inspection "
              "reveals that the egg is lying open, badly damaged.");
    badEgg();
    return RTRUE;
  }
  ...
  if (g.prsa == V_THROW) {
    ...
    printLine("Your rather indelicate handling of the egg has caused it some "
              "damage, although you have succeeded in opening it.");
    badEgg();
    return RTRUE;
  }
  ...
  if (canary && canary->getLocation() == egg) {
    ...
    if (brokenCanary) {
      ...
      printLine(
          ...
          "have recently had a bad experience. The mountings for its "
  ...
  if (g.prsa == V_WIND) {
    if (isCanary) {
      ...
      if (room) {
        ...
        inForest =
            (roomId == RoomIds::FOREST_1 || roomId == RoomIds::FOREST_2 ||
             roomId == RoomIds::FOREST_3 || roomId == RoomIds::FOREST_PATH ||
      ...
      // We can approximate by checking if bauble is not in Initial (Nowhere)
      // state But if player lost it, we might re-spawn? ZIL uses flag. Let's
      // check if bauble has a location. If it's NOWHERE (default), it can
      // spawn.
      ...
      if (inForest && !baubleSpawned) {
        printLine(
            "The canary chirps, slightly off-key, an aria from a forgotten "
            "opera. From out of the greenery flies a lovely songbird. It "
            "perches on a limb just over your head and opens its beak to sing. "
            "As it does so a beautiful brass bauble drops from its mouth, "
            "bounces off the top of your head, and lands glimmering in the "
        ...
        printLine("The canary chirps blithely, if somewhat tinnily, for a "
                  "short time.");
      }
    } else {
      // Broken canary
...
bool boatAction() {
  ...
  if (g.prsa == V_BOARD || g.prsa == V_ENTER) {
    if (isInflated) {
      ...
      ObjectId roomId = room->getId();
      bool isWaterRoom =
          (roomId == RoomIds::RESERVOIR_NORTH ||
           roomId == RoomIds::RESERVOIR_SOUTH || roomId == RoomIds::RESERVOIR ||
           roomId == RoomIds::STREAM_VIEW || roomId == RoomIds::IN_STREAM ||
           roomId == RoomIds::RIVER_1 || roomId == RoomIds::RIVER_2 ||
           roomId == RoomIds::RIVER_3 || roomId == RoomIds::RIVER_4 ||
...
bool pumpAction() {
  ...
  // Handle EXAMINE
  if (g.prsa == V_EXAMINE) {
    printLine("It's a small hand-held air pump, suitable for inflating rubber "
              "objects.");
    return RTRUE;
  }
...
bool machineAction() {
  ...
  if (g.prsa == V_EXAMINE) {
    if (g.prso->hasFlag(ObjectFlag::OPENBIT)) {
      printLine("The machine is a large metal contraption with an open lid. "
                "Inside, you can see a container designed to hold something.");
    } else {
      printLine("The machine is a large metal contraption with a closed lid. "
                "There's a switch on one side.");
    }
    return RTRUE;
  }
...
bool mirrorAction() {
  ...
  bool isMirror = (g.prso && (g.prso->getId() == ObjectIds::MIRROR_1 ||
                              g.prso->getId() == ObjectIds::MIRROR_2));
  if (!isMirror)
    return RFALSE;
...
// Requirements: 42
// DAM-FUNCTION
// ZIL: OPEN/CLOSE -> "Sounds reasonable...". PLUG -> Dutch boy / Tiny leak
// logic. Source: 1actions.zil lines 1400-1410
bool damAction() {
  auto &g = Globals::instance();
  ...
  if (g.prsa == V_PLUG) {
    if (g.prsi && g.prsi->getId() == ObjectIds::HANDS) {
      ...
      printLine("With a " + g.prsi->getDesc() +
                "? Do you know how big this dam is? You could only stop a tiny "
                "leak with that.");
    } else {
...
bool boltAction() {
  ...
  // ZIL: Requires WRENCH. Requires GATE-FLAG (bubble activated). Toggles
  // GATES-OPEN.
  if (g.prsa == V_TURN) {
    // ZIL: <EQUAL? ,PRSI ,WRENCH>
    if (!g.prsi || g.prsi->getId() != ObjectIds::WRENCH) {
      // If turning with something else (or nothing/hands implied if handled by
      // parser) ZIL uses PRSI check. If PRSI is not set, parser usually prompts
      // "Turn with what?" But if we are here, we might need to check
      // validation. Simplified: If checked tool is not Wrench, fail. Actually,
      // parser usually sets PRSI.
    ...
    if (g.gateFlag) {
      if (g.gatesOpen) {
        // <SETG GATES-OPEN <>>
        g.gatesOpen = false;
        printLine("The sluice gates close and water starts to collect behind "
                  "the dam.");
...
bool buttonAction() {
  ...
  if (g.prsa == V_EXAMINE) {
    if (isYellow)
      printLine("It's a small yellow button.");
    else if (isBrown)
      printLine("It's a small brown button.");
    else if (isRed)
      printLine("It's a small red button.");
    else if (isBlue)
      printLine("It's a small blue button.");
    return RTRUE;
  }
  ...
  if (g.prsa == V_PUSH) {
    if (isBlue) {
      if (g.waterLevel == 0) {
        printLine("There is a rumbling sound and a stream of water appears to "
      ...
      g.gateFlag = false;
      printLine("Click.");
      return true; // ZIL also clears DAM-ROOM TOUCHBIT, but that's for
                   // description flow mostly
    } else if (isYellow) {
      g.gateFlag = true;
      printLine("Click.");
      return true;
