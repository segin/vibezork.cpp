...
void loadObjects() {
  ...
  // links every parent's children in reverse definition order.  ZIL's MOVE
  // makes the object the FIRST child of its new parent (ZObject::moveTo
  // prepends), so walking the definitions FORWARDS reproduces that chain, and
  // with it the order FIRST?/NEXT? walks see - which is the order "take all"
  // and the room description use.
  for (auto it = zil::kObjects.begin(); it != zil::kObjects.end(); ++it) {
    const auto &def = *it;
    if (def.in.empty()) {
