...
// ZIL Logic:
// - SWIM / THROUGH: "You can't swim in the stream."
// - CROSS: "The other side is a sheer rock cliff."
// - Other verbs return false
// =============================================================================

TEST(StreamPseudoFcn_SwimReturnsTrue) {
  setupTestWorld();
  ...
  OutputCapture cap;
  bool result = streamPseudo();

  ASSERT_TRUE(result);
  ASSERT_EQ(cap.getOutput(), "You can't swim in the stream.\n");
}

TEST(StreamPseudoFcn_ThroughReturnsTrue) {
  setupTestWorld();
  auto &g = Globals::instance();
  g.prsa = V_THROUGH;
  g.prso = g.getObject(ObjectIds::PSEUDO_OBJECT);
...
TEST(StreamPseudoFcn_CrossReturnsTrue) {
  setupTestWorld();
  auto &g = Globals::instance();
  g.prsa = V_CROSS;
  g.prso = g.getObject(ObjectIds::PSEUDO_OBJECT);
  ...
  ASSERT_TRUE(result);
  ASSERT_EQ(cap.getOutput(), "The other side is a sheer rock cliff.\n");
}

TEST(StreamPseudoFcn_UnhandledVerbsReturnFalse) {
  setupTestWorld();
  auto &g = Globals::instance();
...
TEST(StreamPseudoFcn_NullObjectsSafe) {
  setupTestWorld();
  auto &g = Globals::instance();
  g.prsa = V_CROSS;
  g.prso = nullptr;
...
int main(int argc, char *argv[]) {
  std::cout << "Running StreamPseudo Action Tests" << std::endl;
  std::cout << "=================================" << std::endl;
  std::cout << std::endl;
